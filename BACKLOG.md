...
- [x] Animace projektu na hover — hotovo 2026-05-20
  - `components/Projects.jsx`: oba typy karet (full-bleed `ProjectCard` + grid `HobbyProjectCard`) dostaly `whileHover` lift (-4px / -6px) přes Framer Motion + CSS přechody. Commercial card má pomalý parallax-zoom backgroundu (image/gradient scale 1.04–1.06, 1.2s) a diagonal sheen sweep (-skew-x-12, 1.4s). Hobby card má emerald glow shadow + border highlight, sheen sweep, title slide (translate-x-1) a icon opacity ramp. Vše GPU-accelerated (transform/opacity), žádný re-layout. `group` + `group-hover:` pattern pro koordinaci s child elementy.
