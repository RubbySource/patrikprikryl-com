...
- [x] Kontaktní stránka /contact — samostatná stránka `app/[locale]/contact/page.jsx` se SEO ContactPoint JSON-LD, formulářem (reuse ContactForm komponentu), LinkedIn + GitHub linky, mapa (embed nebo popis lokace Praha). NavLink "Kontakt" do navigace. cs/en/de i18n v namespace `contactPage`. Canonical URL, hreflang tagy. — hotovo 2026-05-24
  - Nová stránka `app/[locale]/contact/page.jsx` (server component): `generateMetadata` s canonical + hreflang (en/cs/de) + OpenGraph/Twitter, `ContactPage` + `ContactPoint` JSON-LD přes nový `contactPageSchema()` v `components/seo/schemas.js`. Formulář vyčleněn do reusable `components/ContactForm.jsx` — sdílí ho homepage `Contact.jsx` i nová stránka (DRY). Pravý panel: LinkedIn + GitHub link karty, lokace Praha (CET) jako popisná karta (místo embed mapy). Navigace: "Kontakt" je nově `<Link>` na /contact (dříve scroll na #contact); `scrollTo` opraven, aby projects/awards fungovaly i z blog/contact stránek (router.push na homepage + hash). Plná i18n cs/en/de — nový namespace `contactPage` + meta klíče `contact_title` / `contact_description` / `contact_tagline`.
