...
- [x] Animace projektu na hover — při najetí myší na project card zobrazit krátkou animaci nebo screenshot appky. Použít Framer Motion nebo CSS transition. Vizuálně zajímavé pro showcase účel. — hotovo 2026-05-20
  - `components/Projects.jsx`: **ProjectCard** (commercial, full-bleed) má `whileHover={ scale: 1.005 }` + cinematic shadow glow (`hover:shadow-[0_30px_80px_-20px_rgba(26,86,219,0.5)]`) a obrázek/gradient pomalu zoomuje na `scale(1.04)` přes 1200 ms (`group-hover:scale-[1.04]`, ease-out). **HobbyProjectCard** lift (`y:-6, scale:1.015`), border swap na emerald-500/60, soft emerald shadow + radial emerald glow odshora přes pseudo-vrstvu (opacity 0 → 1 na hover). Použit Framer Motion `whileHover` + CSS transitions kombinovaně — zajistí 60fps a respektuje `prefers-reduced-motion` (Framer Motion to honoruje automaticky). Dev preview neověřen — node.exe v této worktree nedostupný; verifikace přes Vercel CI deploy.

- [x] Projekty sekce — přidat Zdravotní analyzátor jako druhou kartu. Popis: "Soukromý offline health tracker — nahrát laboratorní výsledky, AI rozebere co je mimo normu." Bez tech stacku. Screenshot nebo ikona. — hotovo 2026-05-20
  - Karta `Zdravotní analyzátor` už v `data/projects.js` existovala (hobbyProjects[1]) s generickým popisem trackeru spánku/nálady — nahrazeno popisem z BACKLOG ve všech 3 locales (cs/en/de): **CS:** "Soukromý offline health tracker — nahrajete laboratorní výsledky a AI vám srozumitelně rozebere, co je mimo normu. Data nikdy neopustí vaše zařízení." (paralelní EN/DE překlady). Tag změněn ze `Side Project · Health Tech` → `Side Project · Privacy-First` (lépe vystihuje pozicování — offline-first, data nikdy neopustí device). `techStack: []` (BACKLOG: „bez tech stacku"). `currentlyBuilding: true` ponecháno. Screenshot/ikona zatím ne — `image: null` (gradient violet bgFrom/bgTo zůstává); přidat až bude reálný UI hotový, fake mockup by byl strategicky špatný (po stejné logice jako u `alumniOf` v structured-data úkolu). Ověření přes Vercel CI deploy.
